...
pub fn markdown(
        ...
        for endpoint in api.endpoints {
            ...
            if let Some(parameters) = endpoint.parameters {
                ...
                for param in parameters {
                    ...
                    r = r + &format!("| `{}` | `.../{}/...` | {} |\n",
                        param.name, param.example, param.description
                    );
                }
                r = r + &format!("\n\n");
            }
            ...
            if let Some(filters) = endpoint.filters {
                ...
                for filter in filters {
                    r = r + &format!("| `{}` | `?...&{}={}...` | {} |\n",
                        filter.name, filter.name, filter.example, filter.description
                    );
                }
                r = r + &format!("\n\n");
            }
            match endpoint.examples.len() {
                ...
                _ => {
                    ...
                    for example in endpoint.examples {
                        ...
                        r = r + &format!("}})\n");
                        r = r + &format!(".then(response => response.json()) // If valid\n");
                        r = r + &format!("//.then(response => response.text()) // If invalid\n");
                        r = r + &format!(".then(json => console.log(json));\n");
                        r = r + &format!(".catch(error => console.error(error));\n");
                        r = r + &format!("```\n\n");
    ...
    if let Some(legends) = collection.legend {
        r = r + &format!("<details><summary><h2>🔧 Details</h2></summary>\n\n");
    ...
    r = r + &format!(
        "<br/>\n\nMade with ❤️ using [api_docs_generator](https://github.com/Jkutkut/rust-api_docs_generator) v{}.\n",
        version
    );
    r
}